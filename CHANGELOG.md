...
- Performance, measured on a 514-file Flutter app: a dry run drops from 35 s to 16 s, and to under 5 s once the cache below is warm, at a third of the peak memory (1.8 GB → 0.6 GB). Sites are ruled out **before** any resolve when they can never verify — `Theme.of(context).x` (the shorthand resolves `of` against the chain's context type, `ColorScheme`), `Colors.red` in a `Color` slot (no supertype of the value declares `red`), `final size = MediaQuery.sizeOf(context)` (no context type at all), likewise `throw`, `as`/`is` operands, interpolations and binary left operands. On Flutter code these were the bulk of the candidates, and each cost one speculative resolve per recovery wave. Every rule is a necessary condition only; what passes is still verified by the resolve. Reverted counts include these sites. Doc-comment references (`[Type.member]`) are no longer collected as sites, lint rules no longer run on speculative resolves, and analysis contexts are located from the roots given rather than per file (the locator spent ~2 ms per file).

- Linked element models — the SDK's, every package's, the project's own libraries' — now persist across runs under the user cache home (`~/Library/Caches/dotsan`, `$XDG_CACHE_HOME/dotsan`, `%LOCALAPPDATA%\dotsan`; 1 GiB, least recently used evicted first). Linking dominates a first run; `--dry-run` followed by the real run, or a second project on the same Flutter SDK, finds the models ready. Entries are keyed by content signature, so a stale entry is never a wrong one, and the directory is safe to delete at any time.

- Sites that a doomed neighbour in an **earlier statement** starved now convert: `final theme = Theme.of(context)` can never convert (an untyped local has no context type), yet applying it speculatively turned `theme` into an invalid type and took every later `theme.copyWith(side: BorderSide.none)` down with it — across the statement boundary the recovery pass relies on, so those sites were never retried. The pre-check never applies such a site, and the rest resolve in round one.
